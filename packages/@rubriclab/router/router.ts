...
export const router = ACTIONS => {
	const route = async (request: NextRequest, response: NextResponse, method: 'GET' | 'POST') => {
		const actions = Object.keys(ACTIONS).map(action => ({name: action, x: ACTIONS[action]}))
		const path = new URL(request.url).searchParams.get('api')
		const action = actions.find(action => action.x.method === method && action.name === path)
	...
	return {GET: (request, response) => route(request, response, 'GET'), POST: (request, response) => route(request, response, 'POST')}
}
